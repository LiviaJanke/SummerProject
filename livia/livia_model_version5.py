...
years,temp_no,temp=np.loadtxt('graph.txt',skiprows=5,unpack=True)
sulph_years, sulphates = np.loadtxt('sulphate_annual_medians_from1880_v2.csv', skiprows = 1, delimiter = ',', unpack = True)
CFC11_years, CFC11 = np.loadtxt('CFC11_1880_to_present.csv', skiprows = 1, delimiter = ',', unpack = True)
time=years[1:]
print(len(time))
surface=510e12
...
def temp_increase_new(number_years):
    ...
    dF_N2O=forcing_n2o(ch4,n2o)
    dF_methane=forcing_methane(ch4,n2o)
    dF_sulph = forcing_sulphates_2(sulphates)
    dF_CFC11 = forcing_CFC11(CFC11)
    for i in range (0,number_years):
        dF_tot=+dF_CO2[i]+dF_N2O[i]+dF_methane[i] + dF_sulph[i] + dF_CFC11[i]
        excess_planetary_energy=(dF_tot-dOLR)*surface
        dT=excess_planetary_energy/mc_tot
        anomaly+=dT
        dOLR=B*anomaly
...
plt.plot(time,temperature_old, color = 'red')
plt.plot(time,temperature_new, color = 'green')
plt.legend()
