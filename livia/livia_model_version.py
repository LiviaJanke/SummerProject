# -*- coding: utf-8 -*-
"""
Created on Tue Jun  1 17:26:48 2021
...
def temp_increase(number_years):
    ...
    dF_methane=forcing_methane(ch4,n2o)
    dT=T_eq-T
    for i in range (1,number_years):
        dF_tot=+dF_CO2[i]+dF_N2O[i]+dF_methane[i]
        print(dF_tot)
        excess_planetary_energy=(dF_tot-dOLR)*surface
        #print(excess_planetary_energy)
        dT=excess_planetary_energy/mc_tot
        #print(dT)
        inc_temp.append(dT)
        dOLR=B*dT
...
#array_temp=yearly_temp_increase(len(time))
temperature_output = temp_increase(140)

#%%
plt.plot(years,temp, color = 'red')
plt.plot(time,temperature_output, color = 'blue', marker = 'x')
